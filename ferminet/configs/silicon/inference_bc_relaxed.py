...
def get_config():
    ...
    # Geometry MUST match training config bc_relaxed.py exactly.
    cfg.system.molecule = [
...
if __name__ == '__main__':
    ...
    # Inference config: restore the trained silicon bc_relaxed checkpoint (#9,
    # job family muon_silicon_q_bc_relaxed) and sample muon positions. No
    # optimisation (optimizer="none"); positions for the first 1000 steps are
    # written to <save_path>/positions/positions_{t}.npy.
    cfg.optim.iterations = 10000000
    cfg.log.save_freq = 2000000
    cfg.log.save_tfreq = 235000
    cfg.log.restore_from_checkpoint = True
    # Trained checkpoints live in the #9 training save dir (latest at launch).
    cfg.log.restore_path = "/projects/u6em/parv/silicon_unpaired/bc_relaxed"
    ...
    cfg.log.save_path = "/projects/u6em/parv/silicon_unpaired/bc_relaxed/inference"
