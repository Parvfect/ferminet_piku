...
def get_config():
    ...
    # Diamond lattice constant (bohr)
    a = 6.74

    # 16 C atoms → 64 valence electrons (with pseudopotential) + 1 quantum muon
    ...
    # 0.0414 bohr, a +0.4% T-cage EXPANSION (contrast silicon's -1.1% contraction).
    # Geometry MUST match training config diamond/t_relaxed/pp.py exactly.
    cfg.system.molecule = [
...
if __name__ == '__main__':
    ...
    # Inference (diamond analogue of EXP-003): restore the trained diamond
    # T-relaxed quantum checkpoint (#12, job family muon_d_qpp_t_rel) and sample
    # muon positions. No optimisation (optimizer="none"); positions for the first
    # 1000 steps are written to <save_path>/positions/positions_{t}.npy. Site /
    # SRPD analysis is computed post-hoc by tools/muon_site_analysis.py. KEY
    # question: does the quantum muon localise in the EXPANDED relaxed T-cage
    # (coordinated by the outward-moved C #4-#7) WITHOUT seeding? (Silicon's muon
    # fled its CONTRACTED relaxed cage; diamond's expansion should hold it.)
    cfg.optim.iterations = 10000000
    cfg.log.save_freq = 2000000
    cfg.log.save_tfreq = 235000
    cfg.log.restore_from_checkpoint = True
    # Trained checkpoints live in the #12 training save dir (latest at launch).
    cfg.log.restore_path = "/projects/u6em/parv/diamond/unpaired/t_relaxed/pp"
    ...
    cfg.log.save_path = "/projects/u6em/parv/diamond/unpaired/t_relaxed/pp/inference"
