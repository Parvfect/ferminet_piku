...
def get_config():
    ...
    # --- EXP-004 VERIFY: same MCMC intervention as wide_burnin.py -------------
    # Muon (last particle) starts at a wide 0.3 proposal, electrons at 0.02. The
    # only differences from wide_burnin.py are a SHORT burn-in and a throwaway
    # save dir: this config exists purely to confirm, on a real compute node,
    # that the deployed train.py prints
    #   "Initial MCMC width per species: [0.02 0.02 0.3]"
    # and writes ckpt_000000 with muon width 0.3 (checked with
    # tools/muon_width_check.py). It is NOT a production run -- delete
    # pp_wide_burnin_verify afterwards and launch the 8-node wide_burnin.py (v3).
    cfg.mcmc.muon_move_width = 0.3   # electrons stay at the default 0.02
    cfg.mcmc.burn_in = 50            # short: we only need the width init, not equilibration
...
if __name__ == '__main__':
    ...
    # Short verification run: write ckpt_000000 (muon width should read 0.3),
    # a couple more checkpoints, then exit. Throwaway save dir.
    cfg.optim.iterations = 201
    cfg.log.save_freq = 100
    cfg.log.save_tfreq = 235
    cfg.log.save_path = "/projects/u6em/parv/diamond/unpaired/bc_relaxed/pp_wide_burnin_verify"
