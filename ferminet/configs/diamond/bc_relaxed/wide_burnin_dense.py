...
if __name__ == '__main__':
    ...
    # Training config
    # EXP-004 dense variant: only the early collapse window is of interest, and
    # save_freq=100 over the full 900k run would write ~9000 x 35MB checkpoints
    # (~300GB). Cap at 10k iters (~100 ckpts, ~3.5GB) -- the production GSE run
    # is the sibling wide_burnin.py. Enough to see the 0->2000 collapse plus
    # margin to confirm the muon never re-diffuses.
    cfg.optim.iterations = 10001
    cfg.log.save_freq = 100  # EXP-004 dense variant: capture the early collapse
    ...
    cfg.log.save_path = "/projects/u6em/parv/diamond/unpaired/bc_relaxed/pp_wide_burnin_dense"
