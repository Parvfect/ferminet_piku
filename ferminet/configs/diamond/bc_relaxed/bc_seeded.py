...
def get_config():
    ...
    # Geometry IDENTICAL to pp_relax_2.py (BC-relaxed cage). The ONLY difference
    # in this run is that the muon walkers are seeded at the BC site (EXP-002).
    cfg.system.molecule = [
...
if __name__ == '__main__':
    ...
    # Fresh net + first-launch seeding: restore_path == save_path, which is empty
    # on first launch, so find_last_checkpoint() returns None and the net trains
    # from scratch ("No checkpoint found. Training new model."). This avoids
    # warm-start contamination from the off-T pp_relax_2 checkpoint (EXP-002 risk
    # #1). Leave cfg.restart.load_data at its default True so restarts CONTINUE
    # the chain (load_data=False would re-seed the muon at BC every restart).
